...
def _load_assets():
    global _CSS,_JS
    if _CSS is None:
        _CSS = open(os.path.join(OUT,'assets','style.css'),encoding='utf-8').read()
        _JS  = open(os.path.join(OUT,'assets','site.js'),encoding='utf-8').read()
    return _CSS,_JS
...
def _style():
    if INLINE:
        css,_=_load_assets(); return '<style>'+css+'</style>'
    return '<link rel="stylesheet" href="/assets/style.css">'
def _script():
    if INLINE:
        _,js=_load_assets(); return '<script>'+js+'</script>'
    return '<script src="/assets/site.js" defer></script>'
